...
#   findings    (read-only) Aggregate unresolved "- [ ]" items across the analysis reports.
#   checkpoint  (git)       Reversible safety snapshot BEFORE fixing (stash, keeps tree).
#   format      (write)     Post-fix: npx prettier --write "src/**/*.{ts,tsx,css,md}".
#
...
function Cmd-Findings {
  ...
  $total = 0
  $map = @{ 'code-review'='code_review_records.md'; 'ui-ux-tester'='ui_ux_tester_records.md' }
  foreach ($src in @('code-review','ui-ux-tester')) {
